...
local function create_bin_dlg()
  ...
  bin_dlg = ProgressDialog(ContextThemeWrapper(this, android.R.style.Theme_DeviceDefault_Dialog))
...
local function binapk(luapath, apkpath)
  ...
  local function tl2lua(tl_path)
    ...
    if file then
      ...
      file:close()
      return tl.loadcode(content)
    end
    return nil
  end

  function writeLua(tl_path)
    local file, err = io.open(string.gsub(tl_path, "%.tl$", ".lua"), "w")
    if not file then
      return nil, "无法打开文件：" .. err
    end

    local content = tl2lua(tl_path)
    file:write(content)
    file:close()
    --os.remove(tl_path)
  end
  ...
  local function addDir(out, dir, f)
    ...
    for n = 0, #ls - 1 do
      ...
      if name==(".using") then
       ...
       elseif name:find("%.tl$") then
        writeLua(luapath .. dir .. name)
        name = string.gsub(name, "%.tl$", ".lua")
        checklib(luapath .. dir .. name)
        local path, err = console.build(luapath .. dir .. name)
        if path then
          if replace["assets/" .. dir .. name] then
          ...
          table.insert(md5s, LuaUtil.getFileMD5(path))
          os.remove(path)
          os.remove(string.gsub(path, "%.luac$", ".lua"))
         else
          table.insert(errbuffer, err)
        end
  ...
  while entry do
    ...
    if replace[name] then
     ...
     elseif name:find("^assets/") and not (name:find "dexopt") then
     ...
     elseif !name:find("%a") then
      ...
      if entry.getName() == "AndroidManifest.xml" then
        ...
        local code = activity.getPackageManager().getPackageInfo(activity.getPackageName(), 0).versionCode
        fo.close()
        local f = io.open(pt, "rb")
        local s = f:read("a")
        f:close()
        -- 跳过 StringPool(0x0001) 与 ResourceMap(0x0180) 两个 chunk 头：
        -- 它们的 size 字段若与 versionCode 数值相同，会被下面的替换误伤，导致 manifest 结构损坏
        local pos = 9
        while true do
          local t = s:byte(pos) + s:byte(pos + 1) * 0x100
          if t ~= 0x0001 and t ~= 0x0180 then
            break
          end
          local a, b, c, d = s:byte(pos + 4, pos + 7)
          pos = pos + a + b * 0x100 + c * 0x10000 + d * 0x1000000
        end
        -- 纯文本查找，避免 versionCode 字节命中 Lua 模式字符
        local i = s:find(touint32(code), pos, true)
        if i then
          s = s:sub(1, i - 1) .. touint32(tointeger(appcode) or 1) .. s:sub(i + 4)
        end
        --s = string.gsub(s, touint32(21), touint32(tointeger(minSdk) or 21))
        --s = string.gsub(s, touint32(29), touint32(tointeger(tarSdk) or 29))

        local f = io.open(pt, "wb")
        f:write(s)
        f:close()
